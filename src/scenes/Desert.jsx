...
export class Desert extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('winImage', '/assets/win.png'); // Load win image
    ...
    this.load.image('raintreeWrong', '/assets/wrong/raintreeWrong.png');
    this.load.image('fragipaniWrong', '/assets/wrong/fragipaniWrong.png');
    this.load.image('jasmineWrong', '/assets/wrong/jasmineWrong.png');
    this.load.image('gulmoharWrong', '/assets/wrong/gulmoharWrong.png');
    ...
    this.load.image('africantulipWrong', '/assets/wrong/africantulipWrong.png');
  ...
  create() {
    ...
    this.add.image(positions.desert.x , positions.desert.y, 'desertBackground')
      .setDisplaySize(habitatSize.width, habitatSize.height);
      
    const headerWidth = 1150; 
    const headerHeight = 500; 
    
    this.add.image(this.sys.game.config.width / 2, 5 , 'headerImage')
      .setOrigin(0.5, 0)
    ...
    const allowedPlants = ['Acacia', 'PricklyPearCactus', 'DatePalm', 'Agave', 'PersianMesquite'];
    const heartRemovingPlants = ['AfricanTulip', 'Gulmohar', 'Jasmine', 'Frangipani', 'RedSilkCotton', 'ArjunaTree', 'RainTree', 'BeachAlmond', 'FlameOfTheForest', 'Teak'];
    ...
    this.input.on('dragend', (pointer, gameObject) => {
      ...
      if (isInDesert && allowedPlants.includes(gameObject.getData('plantKey'))) {
        if (this.plantCount < this.maxPlants) { // Check if the max plant limit is reached
          ...
          const correctedKey = {
            'Acacia': 'AcaciaCorrected',
            'PricklyPearCactus': 'pricklypearcactusCorrected',
            'DatePalm': 'DatePalmCorrected',
            'Agave': 'AgaveCorrected',
            'PersianMesquite': 'PersianMesquiteCorrected'
      ...
      const wrongKey = {
        'AfricanTulip': 'africantulipWrong',
        'Gulmohar': 'gulmoharWrong',
        'Jasmine': 'jasmineWrong',
        'Frangipani': 'fragipaniWrong',
        'RedSilkCotton': 'redsilkcottonWrong',
        'ArjunaTree': 'arjunwrong',
        'RainTree': 'raintreeWrong',
  ...
  update() {
    if (this.hearts.length === 0 && !this.gameOver) {
      ...
      .setOrigin(0.5)
      .setDisplaySize(1200, 700) // Set the width and height for the game over image
      .setDepth(100);
