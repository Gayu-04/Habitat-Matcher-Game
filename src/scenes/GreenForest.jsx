...
export class GreenForest extends Phaser.Scene {

  constructor() {
    super({ key: 'GreenForest' });
  }
  ...
  create() {
    ...
    this.add.image(positions.desert.x , positions.desert.y, 'forestBackground')
      .setDisplaySize(habitatSize.width, habitatSize.height);
      
    const headerWidth = 1150; 
    const headerHeight = 500; 
    
    this.add.image(this.sys.game.config.width / 2, 5 , 'header2Image')
      .setOrigin(0.5, 0)
    ...
    const allowedPlants = ['RedSilkCotton', 'BeachAlmond', 'FlameOfTheForest', 'Teak',  'ArjunaTree'];
    const heartRemovingPlants = ['AfricanTulip', 'Gulmohar', 'Jasmine', 'Frangipani', 'RainTree',  'Acacia', 'PricklyPearCactus', 'DatePalm', 'Agave', 'PersianMesquite'];
    ...
    this.input.on('dragend', (pointer, gameObject) => {
        ...
        if (isInDesert && allowedPlants.includes(gameObject.getData('plantKey'))) {
         ...
         // ✅ Play the correct selection sound
          ...
          const correctedKey = {
            'RedSilkCotton': 'RedSilkCottonCorrected',
            'BeachAlmond': 'BeachAlmondCorrected',
            'FlameOfTheForest': 'FlameOfTheForestCorrected',
            'Teak': 'TeakCorrected',
            'ArjunaTree': 'ArjunaTreeCorrected'
  ...
  update() {
    if (this.hearts.length === 0 && !this.gameOver) {
      ...
      .setOrigin(0.5)
      .setDisplaySize(1200, 700) // Set the width and height for the game over kimage
      .setDepth(100);
